...
namespace Modul_raportow
{
    class ReportGenerator
    {
        ...
        {
            zapytanie = "SELECT CAST( dbo.Schedule.dateFrom AS DATE) AS \"Dzień Pracy\",CAST(dbo.Schedule.dateFrom AS Time(0)) AS \"Czas rozpoczęcia\",CAST(dbo.Schedule.dateTo AS Time(0)) AS \"Czas Zakończenia\", CAST(dbo.Schedule.dateTo - dbo.Schedule.dateFrom AS Time(0)) AS \"Czas pracy\" FROM dbo.Schedule WHERE dbo.Schedule.userId = 1 AND CAST( dbo.Schedule.dateFrom AS DATE) BETWEEN '"+dateFrom.ToString("yyyy-MM-dd")+ "' AND '"+dateFrom.ToString("yyyy-MM-dd")+"';";
